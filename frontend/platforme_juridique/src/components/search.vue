  ...
  >
    <div class="flex gap-2 flex-col mx-auto md:flex-row center wid">
      <div class="relative flex-1">
        ...
        <select
          v-model="this.avocat.idAvocat"
          required
          class="border p-2 rounded w-full"
        >
          <option></option>
          <option v-for="elem in $store.state.avocats" :value="elem.id">
      ...
      <div class="relative flex-1">
        ...
        <select
          v-model="this.avocat.idCategorie"
          required
          class="border p-2 rounded w-full"
        >
          <option></option>

          <option v-for="elem in this.categories" :value="elem.id">
            {{ elem.nom_categorie }}
          </option>
        </select>
      </div>
      ...
      <div class="relative flex-1">
        <button
          @click="search"
          class="bg-white py-3 px-5 text-primary font-semibold rounded-lg hover:shadow-lg transition duration-3000 cursor-pointer"
        >
          Rechercher
        </button>
      </div>
...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      this.getAvocatsBySearch(this.avocat).then((response) => {
