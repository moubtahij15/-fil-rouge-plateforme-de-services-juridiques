...

{
    ...
    public function  getSessionD($id)
    {
        // get consultation info
        $document = new Document();
        $document = $document->read($id);
        // echo json_encode($document);
        // echo json_encode($document['prix']);
        ...
        $checkout = $stripe->checkout->sessions->create([
            ...
            'line_items' => [
                [
                    'price_data' => [
                        'currency' => 'mad',
                        'unit_amount' => $document[0]["prix"] * 100,
                        'product_data' => [
                            'name' => "consultation " . $document[0]["nom_document"]
                        ]
                    ],
